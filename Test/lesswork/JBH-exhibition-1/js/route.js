...
require.config({
  ...
  paths: {
    ...
    mmState: 'lib/mmState',
    mmRequest: 'lib/mmRequest',
    mmHistory: 'lib/mmHistory',
    mmPromise: 'lib/mmPromise',
...
//      model.area.city_code = model.area.city_list[0].ccode;
      model.area.city_code = 'all';
    }
    model.setQuery({province_ccode: val == 'all' ? null : val});
  });
...
//      model.area.area_code = model.area.area_list[0].ccode;
      model.area.area_code = 'all';
    }
    model.setQuery({city_ccode: val == 'all' ? null : val});
  });
  model.area.$watch('area_code', function (val) {
    model.setQuery({area_ccode: val == 'all' ? null : val});
  });


  model.selected.$watch('business_model', function (val) {
    model.setQuery({business_model: val == 'all' ? null : val});
  });
  ...
  avalon.state("index", {
    ...
    onEnter: function () {
          ...
          if ($("#carousel1").length > 0 || count > 60) {
            ...
            $('.sider-trigger').bind({
              'mouseenter': function () {
                ...
                $('#' + tid).find('.cloudcarousel').trigger('mouseover', true);
              }
            });
      ...
      Api.get('index-main', query, function (res) {
        ...
        // 单行公告上下轮换
        var $ul = $('.announcement .wrapper ul');
        ...
        $ul.delegate('li', {
          ...
          'mouseleave': function () {
            timer = setInterval(interval, 3500);
          }
        });
        function interval() {
          if ($ul.find('li').length > 1) {
            $ul.find('li').eq(0).animate({
              ...
              $(this).appendTo($ul).css({marginTop: 0});
            })
          }
        }
  ...
  avalon.state("product-search", {
    ...
    onEnter: function () {
      var query = avalon.mix({}, model.query = this.query, {page_size: 20});
      query.name = query.s || '';
      model.common = {
        title: '产品搜索'
      };
...
//      });
  ...
  avalon.state("information-list", {
    ...
    onEnter: function () {
      var query = avalon.mix({}, model.query = this.query, { page_size: 10 });
      model.common = {
        title: '资讯列表页'
      };
